...
public class P002122 {
	...
	public static void main(String[] args) throws FileNotFoundException {
		PrintStream fl = new PrintStream(new File("src/POO2122/POO_2122_EP.txt")); //ALTEREI PARA CORRESPONDER A MINHA PASTA
	...
	private static void alinea1(PrintStream out) {
		...
		e2.addActivity(new Catering(Catering.Option.LIGHT_BITES, 20))
			.addActivity(new Culture(Culture.Option.ARCHITECTURAL_TOUR, 12))
			.addActivity(new Culture(Culture.Option.RIVER_TOUR, 12));

		Event e3 = grandesEventos.addEvent(c3, LocalDate.parse("2022-06-25"));
		e3.addActivity(new Catering(Catering.Option.LIGHT_BITES, 20))
			.addActivity(new Culture(Culture.Option.WINE_TASTING, 20));

		Event e4 = grandesEventos.addEvent(c1, LocalDate.parse("2022-07-10"));
		e4.addActivity(new Sport(Sport.Modality.BIKE, 16))
			.addActivity(new Culture(Culture.Option.WINE_TASTING, 20));
	...
	private static void alinea2(PrintStream out) {
		...
		// 	for (String s: em.getAllClientsWithEvents())  // devolve todos os clientes com eventos agendados
		// 		out.println(s); 
		// 	out.println("\n" + em + ": LISTA DE PRÓXIMOS EVENTOS ORDENADOS POR DATA");	
		// 	for (String s: em.getAllEventsByDate())  // devolve todos os eventos agendados, ordenados por data
		// 		out.println(s); 
		// 	out.println();
		// }
	}
